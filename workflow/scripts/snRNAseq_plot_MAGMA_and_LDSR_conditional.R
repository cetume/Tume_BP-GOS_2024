...
magma_LRRK1 <- 'Herring_snRNAseq_2023_pipeline/results/magma_conditional/snRNAseq.herring_all_sig_condition_L4_RORB_LRRK1.lvl2.magma.35UP_10DOWN.gsa.out'
...
magma_MET <- 'Herring_snRNAseq_2023_pipeline/results/magma_conditional/snRNAseq.herring_all_sig_condition_L4_RORB_MET.lvl2.magma.35UP_10DOWN.gsa.out'
...
MAGMA_LDSR_MEAN_PLOT <- ggplot(data = PLOT_mean, aes(x = mean, y = factor(Category, rev(levels(factor(Category)))), fill = COLOUR)) +
  geom_bar(stat = "identity", color = 'black', position = "dodge") +
  scale_fill_manual(values = colour_table$Code, drop = FALSE, name = "Significant at Bonferroni threshold") +
  geom_vline(xintercept=-log10(BF_CORR), linetype = "dashed", color = "black") +
  geom_vline(xintercept=-log10(0.05), linetype = "dotted", color = "black") +
  theme_bw() +
  theme(plot.margin = unit(c(0.5, 0.5, 0.5, 0.5), "cm"),
